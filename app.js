...
class KeycapPreview {
  ...
  async init() {
    ...
    document
      .getElementById('generateNoteBtn')
      .addEventListener('click', () => this.generateAndCopyNote())
  ...
  loadSingleLayer(layerName) {
    return new Promise((resolve, reject) => {
      ...
      img.onerror = () => {
        console.error(
          `图层 ${layerName} 加载失败: ${CONFIG.layerPaths[layerName]}`
        )
  ...
  getComplementaryColor(hex) {
    ...
    const complementaryRgb = this.hslToRgb(
      complementaryHue,
      newSaturation,
      newLightness
    )
  ...
  drawWatermark() {
    ...
    // 检查字体是否可用
    const stapelAvailable =
      this.fontsLoaded.stapel ||
    ...
    for (let i = 0; i < text.length; i++) {
      ...
      const color =
        this.currentColors[colorLetter] || CONFIG.originalColors[colorLetter]
  ...
  loadSavedSchemesList() {
    ...
    container.innerHTML = schemes
      .map((scheme, index) => {
        ...
        const year =
          scheme.textConfig && scheme.textConfig.year
            ? scheme.textConfig.year
            : ''
        const word =
          scheme.textConfig && scheme.textConfig.word
            ? scheme.textConfig.word
            : ''
        const yearWordInfo =
          year || word
            ? `<div class="scheme-meta">年份: ${year || '-'} | 单词: ${
                word || '-'
              }</div>`
            : ''
        return `
                <div class="scheme-item">
                    <div class="scheme-info">
                        <div class="scheme-name">${this.escapeHtml(
                          scheme.name
                        )}</div>
                        ...
                        <div class="scheme-colors">
                            ${['A', 'B', 'C', 'D', 'E', 'F']
                              .map((letter) => {
                                ...
                                const number =
                                  scheme.colorNumbers &&
                                  scheme.colorNumbers[letter]
                                return `<span class="color-tag">${letter}: ${
                                  number ? `色号${number}` : color
                                }</span>`
                              })
                              .join('')}
                        </div>
  ...
  async initPresets() {
    try {
      const response = await fetch(CONFIG.presetsPath)
      if (!response.ok) {
        throw new Error('预设方案加载失败')
      }
      ...
      if (this.colorCardColors) {
        this.matchPresetColorsToPalette()
      } else {
        // 如果色卡未加载，等待加载完成
        setTimeout(() => {
          if (this.colorCardColors) {
  ...
  // 将预设方案中的颜色匹配到色板
  matchPresetColorsToPalette() {
    if (!this.presets || !this.colorCardColors) return

    this.presets.forEach((preset) => {
      ;['A', 'B', 'C', 'D', 'E', 'F'].forEach((letter) => {
        const originalColor = preset.colors[letter]
        if (originalColor) {
          const match = this.findClosestColorInPalette(originalColor)
          if (match) {
            // 更新为色板中的实际颜色和色号
            preset.colors[letter] = match.hex
            if (!preset.colorNumbers) {
              preset.colorNumbers = {}
            }
            preset.colorNumbers[letter] = match.number
          }
        }
      })
    })
  ...
  renderPresets() {
    ...
    // 移除检查，允许在模态框外渲染（模态框打开时会调用）

    if (!this.presets || this.presets.length === 0) {
      container.innerHTML = '<div class="presets-loading">暂无预设方案</div>'
      return
    }

    // 根据选中的category筛选预设方案
    const filteredPresets = this.selectedCategory
      ? this.presets.filter(
          (preset) => preset.category === this.selectedCategory
        )
      : this.presets

    if (filteredPresets.length === 0) {
      container.innerHTML =
        '<div class="presets-loading">该类别下暂无预设方案</div>'
      return
    }
    ...
    filteredPresets.forEach((preset, filteredIndex) => {
      const originalIndex = this.presets.findIndex((p) => p.id === preset.id)
      originalIndexMap.set(filteredIndex, originalIndex)
    })

    container.innerHTML = filteredPresets
      .map((preset, filteredIndex) => {
        const colors = preset.colors || {}
        const textConfig = preset.textConfig || {}
        const year = textConfig.year || ''
        const word = textConfig.word || ''
        const wordSpace = textConfig.wordSpace || ''
        ...
        // 获取颜色
        const colorB = colors.B || '#FFFFFF'
        const colorA = colors.A || '#000000'
        const colorC = colors.C || '#FFFFFF'
        const colorD = colors.D || '#FFFFFF'
        const colorE = colors.E || '#FFFFFF'
        const colorF = colors.F || '#FFFFFF'

        return `
                <div class="preset-item" data-preset-id="${preset.id}" 
                     style="background-color: ${colorB}; color: ${colorA};">
                    <div class="preset-item-bg-stripes">
                        <div class="preset-stripe stripe-c" style="background-color: ${colorC};"></div>
                        <div class="preset-stripe stripe-d" style="background-color: ${colorD};"></div>
                        <div class="preset-stripe stripe-e" style="background-color: ${colorE};"></div>
                        <div class="preset-stripe stripe-f" style="background-color: ${colorF};"></div>
                    </div>
                    <div class="preset-item-content">
                        <div class="preset-header">
                            <div>
                                <div class="preset-name">${this.escapeHtml(
                                  preset.name
                                )}</div>
                                <span class="preset-category">${this.escapeHtml(
                                  preset.category || ''
                                )}</span>
                            </div>
                        </div>
                        <div class="preset-description">${this.escapeHtml(
                          preset.description || ''
                        )}</div>
                    <div class="preset-colors">
                        ${['A', 'B', 'C', 'D', 'E', 'F']
                          .map((letter) => {
                            const color = colors[letter] || '#FFFFFF'
                            const labelColor = getLabelColor(color)
                            const colorNumber =
                              preset.colorNumbers && preset.colorNumbers[letter]
                                ? preset.colorNumbers[letter]
                                : ''
                            return `
                                <div class="preset-color-swatch" style="background-color: ${color};" title="${letter}: ${color}${
                              colorNumber ? ' (色号' + colorNumber + ')' : ''
                            }">
                                    <span class="preset-color-label" style="color: ${labelColor};">${letter}</span>
                                </div>
                            `
                          })
                          .join('')}
                        ...
                        <div class="preset-meta">
                            ${year ? `<span>年份: ${year}</span>` : ''}
                            ${word ? `<span>关键词: ${word}</span>` : ''}
                            ${
                              wordSpace
                                ? `<span>关键词2: ${wordSpace}</span>`
                                : ''
                            }
                        </div>
                    </div>
    ...
    container
      ...
      .forEach((item, filteredIndex) => {
        item.addEventListener('click', (e) => {
          // 如果点击的是颜色色块或标签，不触发加载
          if (
            e.target.classList.contains('preset-color-swatch') ||
            e.target.classList.contains('preset-color-label') ||
            e.target.closest('.preset-color-swatch')
          ) {
            return
          }
          // 使用原始索引加载预设方案
          const originalIndex = originalIndexMap.get(filteredIndex)
          if (originalIndex !== undefined) {
            this.loadPreset(originalIndex)
          }
        })
      })
  }
  ...
  // 加载预设方案
  loadPreset(index) {
    if (!this.presets || index < 0 || index >= this.presets.length) {
      this.showToast('预设方案不存在', 'error')
      return
    }
    ...
    // 应用颜色和色号
    ;['A', 'B', 'C', 'D', 'E', 'F'].forEach((letter) => {
      const color = preset.colors[letter]
      if (color) {
        this.currentColors[letter] = color
        this.colorInputs[letter].value = color

        // 应用色号（如果预设方案中有）
        if (preset.colorNumbers && preset.colorNumbers[letter]) {
          this.colorNumbers[letter] = preset.colorNumbers[letter]
        } else {
          this.colorNumbers[letter] = null
        }
      }
    ...
    // 应用文字配置
    if (preset.textConfig) {
      this.textConfig.year = preset.textConfig.year || ''
      this.textConfig.word = preset.textConfig.word || ''
      this.textConfig.wordSpace = preset.textConfig.wordSpace || ''
    ...
    this.showToast(`预设方案 "${preset.name}" 已加载！`, 'success')
  }
  ...
  initPresetsModal() {
    ...
    if (categoryFilter) {
      ...
      categoryFilter.addEventListener('click', (e) => {
        const tag = e.target.closest('.preset-category-tag')
        if (tag) {
    ...
    if (closePresetsModal) {
      closePresetsModal.addEventListener('click', closeHandler)
    }
  ...
  updateCategoryFilter() {
    const categoryFilter = document.getElementById('presetCategoryFilter')
    if (!categoryFilter || !this.presets) return

    // 获取所有唯一的category
    const categories = [
      ...new Set(this.presets.map((preset) => preset.category).filter(Boolean)),
    ]
    ...
    categories.sort().forEach((category) => {
      ...
      tag.className = 'preset-category-tag'
    ...
    if (activeTag) {
      ...
      // 如果没有找到对应的tag，默认选中"全部"
      const allTag = categoryFilter.querySelector('[data-category=""]')
      if (allTag) {
        categoryFilter.querySelectorAll('.preset-category-tag').forEach((t) => {
          t.classList.remove('active')
        })
  ...
  initColorCardPicker() {
    // 为色卡Canvas添加点击事件
    this.colorCardCanvas.addEventListener('click', (e) =>
      this.handleColorCardClick(e)
    )

    // 添加鼠标移动事件（放大镜效果）
    this.colorCardCanvas.addEventListener('mousemove', (e) =>
      this.handleColorCardMouseMove(e)
    )
    this.colorCardCanvas.addEventListener('mouseenter', () =>
      this.showMagnifier()
    )
    this.colorCardCanvas.addEventListener('mouseleave', () =>
      this.hideMagnifier()
    )
  }
  ...
  searchClosestColor(hexValue) {
    ...
    if (closestColorNumber !== null) {
      ...
      this.showToast(
        `找到最接近的色号：${closestColorNumber} (${closestColorHex})`,
        'success'
      )
    } else {
  ...
  // 查找色板中最接近的颜色（返回色号和颜色）
  findClosestColorInPalette(hexValue) {
    const normalizedHex = this.normalizeHexColor(hexValue)
    const colorCardColors = this.extractColorCardColors()
    ...
    if (closestColorNumber !== null) {
      return {
        number: closestColorNumber,
        hex: colorCardColors[closestColorNumber],
      }
    }
  ...
  async loadColorCards() {
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        ...
        // 绘制图片
        this.colorCardContext.drawImage(
          img,
          0,
          0,
          this.colorCardCanvas.width,
          this.colorCardCanvas.height
        )
        ...
        try {
          ...
          console.log(
            '色卡颜色提取完成，共',
            Object.keys(this.colorCardColors || {}).length,
            '个色号'
          )
  ...
  handleColorCardClick(event) {
    ...
    if (colorNumber && this.currentSelectingLetter) {
      ...
      // 立即应用颜色
      this.applyColorFromCard(
        this.currentSelectingLetter,
        colorHex,
        colorNumber
      )
    } else {
  ...
  handleColorCardMouseMove(event) {
    if (
      !this.magnifierVisible ||
      !this.colorCardMagnifier ||
      !this.colorCardImage
    ) {
      return
    }
    ...
    sourceX = Math.max(
      0,
      Math.min(this.colorCardImage.width - sourceWidth, sourceX)
    )
    sourceY = Math.max(
      0,
      Math.min(this.colorCardImage.height - sourceHeight, sourceY)
    )
    ...
    // 计算颜色编号
    const colorNumber = this.getColorNumberFromClick(
      Math.floor(canvasX),
      Math.floor(canvasY)
    )

    // 更新放大镜中显示的颜色信息
    const colorInfo = this.colorCardMagnifier.querySelector(
    ...
    return new Promise((resolve) => {
      ...
      Promise.all([stapelFont.load(), frestyFont.load()])
        ...
        .then(() => {
          ...
          const stapelCheck1 = document.fonts.check(
            '20px "Stapel Narrow Extra Bold Italic"'
          )
          const stapelCheck2 = document.fonts.check(
            '1em "Stapel Narrow Extra Bold Italic"'
          )
          this.fontsLoaded.stapel = stapelCheck1 || stapelCheck2

          const frestyCheck1 = document.fonts.check(
            '30px "Fresty Personal Use Only"'
          )
          const frestyCheck2 = document.fonts.check(
            '1em "Fresty Personal Use Only"'
          )
          this.fontsLoaded.fresty = frestyCheck1 || frestyCheck2

          // 如果检查失败，检查字体是否在文档中
          if (!this.fontsLoaded.fresty) {
            const frestyInFonts = Array.from(document.fonts).some(
              (font) =>
                font.family === 'Fresty Personal Use Only' &&
                font.status === 'loaded'
            )
            if (frestyInFonts) {
          ...
          if (!this.fontsLoaded.stapel) {
            const stapelInFonts = Array.from(document.fonts).some(
              (font) =>
                font.family === 'Stapel Narrow Extra Bold Italic' &&
                font.status === 'loaded'
            )
            if (stapelInFonts) {
              console.log(
                'Stapel字体在文档中找到，但check()返回false，将尝试使用'
              )
          ...
          console.log('字体加载完成', {
            ...
            allFonts: Array.from(document.fonts).map((f) => ({
              family: f.family,
              status: f.status,
            })),
          })
  ...
  drawUserText() {
    ...
    const stapelAvailable =
      ...
      document.fonts.check('20px "Stapel Narrow Extra Bold Italic"') ||
      document.fonts.check('1em "Stapel Narrow Extra Bold Italic"')
    const frestyAvailable =
      this.fontsLoaded.fresty ||
    ...
    const wordToDraw =
      this.textConfig.wordSpace && this.textConfig.wordSpace.trim()
        ? this.textConfig.wordSpace
  ...
  generateAndCopyNote() {
    ...
    if (missingColors.length > 0) {
      this.showToast(
        `请先为以下颜色选择色卡编号：${missingColors.join(', ')}`,
        'warning'
      )
      return
    }
